...
int list_equals(List A, List B)
{
	...
	equal_to = ( length(A) == length(B) );
	A_node = A->front;
	B_node = B->front;
	while( equal_to && A_node != NULL)
	{
		equal_to = ( A_node->data == B_node->data);
		A_node = A_node->next; //moves down the list and extracts its field
		B_node = B_node->next;
	}
	return equal_to;
}
...
void set(List L, long x)
{
	...
	if( index(L) < 0 )
	{
		fprintf(stderr, "List Error: calling set() on NULL cursor reference\n");
		exit(EXIT_FAILURE);
	}
	L->cursor->data = x;
}
...
{
	long value;
	moveFront(L); //move the cursor to the front of the list
	while(index(L) >= 0)
	{
		value = get(L);
...
List copyList(List L)
{
	...
	Node current = NULL;
	for(current = L->front; current != NULL; current = current->next)
	{
